...
import com.task.smartdubai.DataStatus
import com.task.R
...
class ArticlesListActivityTest {
    ...
    fun displayArticlesList() {
        ...
        onView(withId(R.id.rv_articles_list)).check(matches(isDisplayed()))
    ...
    fun noData() {
        ...
        mActivityTestRule.launchActivity(null)
        onView(withId(R.id.rv_articles_list)).check(matches(not(isDisplayed())))
        onView(withId(R.id.pb_loading)).check(matches(not(isDisplayed())))
    ...
    fun testSearch() {
        ...
        onView(isAssignableFrom(AutoCompleteTextView::class.java))
                .perform(typeText(articles.results[1].title))
    ...
    fun testScroll() {
        dataStatus = DataStatus.Success
        mActivityTestRule.launchActivity(null)
        onView(withId(R.id.rv_articles_list))
